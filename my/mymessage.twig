...
<div id="msg{{ name }}" class="callout callout-{{ classname|default( 'info' ) }} fade in" style="margin-bottom:0px">
    ...
    {% for el in elements %}
        ...
        {% elseif el.type == 'button' %}
            <a style="margin:5px 3px 0px 0px{% if el.colorbackground %};background-color:{{el.colorbackground}}{%endif%}{% if el.color %};color:{{el.color}}{%endif%}" type="button" class="btn btn-{{ el.class|default( classname ) }}" {{ el.urlobj|urlobj }}><i class="{{ el.icon }}"></i> {{ el.label }}</a>
